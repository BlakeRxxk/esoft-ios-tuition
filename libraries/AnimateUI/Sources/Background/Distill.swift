...
class Distill {
  ...
  static func bezier(_ property: Animation.Property,
                     bezierPoints: [Float],
  ...
  func calculateSpring(_ property: Animation.Property,
                       finalValue: NSValue,
                       layer: CALayer,
                       type: Animation.Spring,
                       spring: SpringOptions) -> [NSValue] {
  ...
  fileprivate func springStatusEnded(_ previous: CGFloat, proposed: CGFloat, to: CGFloat, increment: CGFloat) -> Bool {
    abs(proposed - previous) <= increment && abs(previous - to) <= increment
  }
}
