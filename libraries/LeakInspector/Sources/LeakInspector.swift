...
@objc public protocol LeakInspectorDelegate {
...
@objcMembers
public class LeakInspector: NSObject {
  ...
  private class RefWatch: CustomStringConvertible {
    ...
    var description: String {
      "LeakInspector.RefWatch(name: \(name), ignore: \(ignore), failedChecks: \(failedChecks))"
    }
  ...
  static public var delegate: LeakInspectorDelegate? {
    didSet {
  ...
  private func ignoreClass(_ type: AnyObject.Type) {
    for (index, clazz) in classesToIgnore.enumerated() where clazz === type {
  ...
  private func unregister(_ ref: AnyObject) {
    for (index, aRefWatch) in refsToWatch.enumerated() where aRefWatch.ref === ref {
  ...
  private func shouldWatch(_ ref: AnyObject) -> Bool {
    ...
    for refWatch in refsToWatch where ref === refWatch.ref {
  ...
  private func checkForLeaks() {
    ...
    for refWatch in removeRefs {
      for (index, aRefWatch) in refsToWatch.enumerated() where refWatch === aRefWatch {
