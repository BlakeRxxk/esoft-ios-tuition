...
public final class MessageSectionController: ListSectionController {
  ...
  override public func sizeForItem(at index: Int) -> CGSize {
    ...
    if isFirstSection {
      height = 38 + text.styledLabelHeight(with: width - 32) { label in
        label.setStyles(UILabel.Styles.small,
                        UILabel.Styles.multiline)
      }
    } else {
      height = text.styledLabelHeight(with: width - 32) { label in
        label.setStyles(UILabel.Styles.tiny,
                        UILabel.Styles.multiline)
      }
    }
